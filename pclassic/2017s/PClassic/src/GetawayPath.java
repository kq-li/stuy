  ...
  /** This is for you to fill out. 
	...
	*/
	...
	public static int longestPath(int[][] matrix) {
		...
		for(int i = 1; i <= N * N; i++) adj[i] = new ArrayList<Integer>();
		...
		int result = 0;
		for(int index = ordering.size() - 1; index >= 0; index--)
		{
			int currentV = ordering.get(index);
			int maxCount = 1;
			for(Integer v2 : adj[currentV])
			{
				maxCount = Math.max(maxCount, DP[v2] + 1);
			}
			DP[currentV] = maxCount;
			result = Math.max(result, DP[currentV]);
		}
